...
- (void)updatePageIndicator
{
    for (int i = 0; i < [self.subviews count]; i++)
    {
        ...
        dot.layer.borderColor = (self.tintColor?:[UIColor lightGrayColor]).CGColor;
        if(i == self.currentPage){
            dot.backgroundColor = self.tintColor ?:[UIColor lightGrayColor];
        }else{
            dot.backgroundColor = self.normalColor ?: [UIColor clearColor];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self){
        ...
        self.bannerPlayView.backgroundColor = [UIColor whiteColor];
...
- (void)setTintIndicatorColor:(UIColor *)tintIndicatorColor{
    self.pageControl.tintColor = tintIndicatorColor;
}

- (void)setNormalIndicatorColor:(UIColor *)normalIndicatorColor{
    self.pageControl.normalColor = normalIndicatorColor;
}
